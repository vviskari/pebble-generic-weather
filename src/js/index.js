...
//var owmforecast = require('./owmforecast.json');
//var owmweather = require('./owmweather.json');
...
var GenericWeather = function() {
    ...
    this._getWeatherOWM = function(coords) {
        ...
        function parseOwmIcon(iconString) {
            var condition = parseInt(iconString.substring(0, 2), 10);
            switch (condition) {
                case 1:
        ...
        console.log('weather: Contacting OpenWeatherMap.org... ' + weatherUrl);
        // console.log(weatherUrl);

        this._xhrWrapper(weatherUrl, 'GET', function(req) {
            console.log('weather: Got weather API response!');
            if (req.status == 200) {
                ////////
                // req.response = JSON.stringify(owmweather);
                /////////
                var json = JSON.parse(req.response);

                var message = {};
                message[keys.GW_REPLY] = true;
                message[keys.GW_TEMPK] = Math.round(json.main.temp);
                message[keys.GW_NAME] = json.name;
                message[keys.GW_DESCRIPTION] = json.weather[0].description;
                message[keys.GW_DAY] = (json.dt > json.sys.sunrise && json.dt < json.sys.sunset) ? true : false;
                message[keys.GW_CONDITIONCODE] = parseOwmIcon(json.weather[0].icon);
                message[keys.GW_SUNRISE] = json.sys.sunrise;
                message[keys.GW_SUNSET] = json.sys.sunset;
                ...
                if (this._forecast) {
                    ...
                    console.log('weather: Contacting OpenWeatherMap.org... ' + forecastUrl);

                    this._xhrWrapper(forecastUrl, 'GET', function(req) {
                        console.log('weather: Got forecast API response!');
                        if (req.status == 200) {
                            ///////
                            // req2.response = JSON.stringify(owmforecast);
                            ////////
                            var forecastJson = JSON.parse(req.response);
                            var forecasts = [];
                            forecastJson.list.forEach(function(forecast) {
                                forecasts.push({
                                    temp: forecast.main.temp,
                                    time: parseInt(forecast.dt),
                                    condition: parseOwmIcon(forecast.weather[0].icon)
                                });
                            });
                            for (var i = 0; i < forecasts.length && i < 18; i++) {
                                message[keys.GW_FORECAST_TEMPK + i] = forecasts[i].temp;
                                message[keys.GW_FORECAST_TIME + i] = forecasts[i].time;
                                message[keys.GW_FORECAST_CONDITIONCODE + i] = forecasts[i].condition;
                            }
                        } else {
                            console.log('weather: Error fetching forecast data (HTTP Status: ' + req.status + ')');
                        }
                ...
                console.log('weather: Error fetching weather data (HTTP Status: ' + req.status + ')');
    ...
    this._getWeatherWU = function(coords) {
        ...
        function parseWuIcon(condition) {
        ...
        this._xhrWrapper(url, 'GET', function(req) {
            ...
            if (req.status == 200) {
                ...
                message[keys.GW_CONDITIONCODE] = parseWuIcon(json.current_observation.icon);
                ...
                if (this._forecast && json.hourly_forecast) {
                    ...
                    json.hourly_forecast.forEach(function(forecast) {
                        forecasts.push({
                            ...
                            condition: parseWuIcon(forecast.icon)
                        });
                    });
    ...
    this._getWeather = function(coords) {
        switch (this._provider) {
            case GenericWeather.ProviderOpenWeatherMap:
                this._getWeatherOWM(coords);
                break;
            case GenericWeather.ProviderWeatherUnderground:
                this._getWeatherWU(coords);
                break;
            case GenericWeather.ProviderForecastIo:
                this._getWeatherF_IO(coords);
                break;
            case GenericWeather.ProviderYahooWeather:
                this._getWeatherYahoo(coords);
                break;
            default:
                break;
        }
